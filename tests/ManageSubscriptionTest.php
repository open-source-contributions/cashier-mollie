...
class ManageSubscriptionTest extends BaseTestCase
{
    ...
    public function canCreateDirectDebitSubscriptionForMandatedCustomer()
    {
        ...
        $this->assertSame(0, $user->orderItems()->count());
        ...
        $this->assertSame(1, $user->subscription('main')->orderItems()->count());
        $this->assertSame(1, $user->orderItems()->count());
        ...
        $this->assertSame('EUR', $scheduled_order_item->currency);
        $this->assertEquals($user->id, $scheduled_order_item->owner_id);
        $this->assertInstanceOf($scheduled_order_item->owner_type, $user);
        $this->assertCarbon(now(), $scheduled_order_item->process_at, 5);
        $this->assertSame(1, $scheduled_order_item->quantity);
        $this->assertSame(1000, $scheduled_order_item->unit_price);
        $this->assertSame(1000, $scheduled_order_item->subtotal);
        $this->assertSame(0, $scheduled_order_item->discount_value); // TODO implement discounts
        $this->assertSame(10, $scheduled_order_item->tax_percentage);
        $this->assertSame('Monthly payment', $scheduled_order_item->description);
        ...
        $this->assertSame(1, $scheduled_order_item->quantity);
        $this->assertSame(1000, $scheduled_order_item->subtotal);
        $this->assertSame(0, $scheduled_order_item->discount); // TODO implement discounts
        $this->assertSame(100, $scheduled_order_item->tax);
        $this->assertSame(1100, $scheduled_order_item->total);
        $this->assertFalse($scheduled_order_item->isProcessed());
        ...
        Event::assertDispatched(SubscriptionQuantityUpdated::class, function (SubscriptionQuantityUpdated $e) use ($subscription) {
            $this->assertTrue($e->subscription->is($subscription));
            $this->assertSame(2, $e->subscription->quantity);
            $this->assertSame(1, $e->oldQuantity);
            return true;
        });
        ...
        Event::assertDispatched(SubscriptionQuantityUpdated::class, function (SubscriptionQuantityUpdated $e) use ($subscription) {
            $this->assertTrue($e->subscription->is($subscription));
            $this->assertSame(1, $e->subscription->quantity);
            $this->assertSame(2, $e->oldQuantity);
            return true;
        });
        ...
        $this->assertSame('monthly-10-2', $subscription->plan);
        // Other swap characteristics are covered in SwapSubscriptionPlanTest.php

        // Invoice Tests
        $invoice = $user->invoices()[0];

        $this->assertSame('11,00 €', $invoice->total());
